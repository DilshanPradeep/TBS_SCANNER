...
@app.route('/api/get_timer_state', methods=['GET'])
def get_timer_state():
    ...
    try:
        if os.path.exists(timer_file):
            ...
            state = {"timer_enabled": False, "last_updated": None}
            with open(timer_file, 'w') as f:
                json.dump(state, f)
        return jsonify(state)
    except Exception as e:
        print(f"Error reading timer state: {str(e)}")
        return jsonify({"timer_enabled": False, "last_updated": None})

@app.route('/api/set_timer_state', methods=['POST'])
def set_timer_state():
    """Set the timer state"""
    timer_file = "timer_state.json"
    try:
        ...
        print(f"Error setting timer state: {str(e)}")
